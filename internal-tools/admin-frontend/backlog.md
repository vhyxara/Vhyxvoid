...
- [ ] **Production has no admin user, ability or role rows** (the whole database is empty), so no one can log in to the admin panel there. `seed-super-admin.ts` creates `admin@company.local` / `Admin@12345678` and there is no admin password change/reset endpoint, so do not run it as-is in production: parameterize the email/password (env) first, then run `seed-abilities`, `seed-roles`, `seed-super-admin` against production deliberately.
