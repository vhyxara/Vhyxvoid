...
- [ ] **`@vhyxvoid/middleware` assumes port 3000 (audit part2 G8).** It can detect the app's real port from the first request (`req.socket.localPort`) or `fastify.server.address()`; today a mismatched port gives silent 502s. See `shared/audit-2026-09-24-part2.md` G8. Filed 2026-09-25 from audit part2 (session 2026-09-25-audit-part2-fixes). **Update 2026-09-25 (session 2026-09-25-backlog-sweep):** not done. Express middleware can only see the port on the first request, after the tunnel has already registered with the default, so a fix means restarting the tunnel on the first request (or a `setPort` on AgentClient/BackendProxy); Fastify can use the `onListen` hook. Needs a decision on which.
  Resolved 2026-09-25, 4a3131f (session 2026-09-25-decisions-and-e2e), AgentClient.setPort; middleware adopts the real port (Express first request, Fastify onListen) unless set explicitly; code-archive/shared/CA-0026.
- [ ] **The agent's SQLite queue has no producer.** Nothing calls `DurableQueue.enqueueInbound`, and since `2f90df4` (audit part2 G3) outbound responses aren't persisted either, so the CLI agent opens `~/.vhyxvoid/queue.db` (and needs the `better-sqlite3` native module) only to purge leftovers. Decide: delete `DurableQueue`/`replayQueue`/`better-sqlite3` (a large install-time win, a public-API removal) or wire inbound replay for real (needs a hub-side design: the hub never re-sends a `tunnel:forward`). Found 2026-09-25, session 2026-09-25-audit-part2-fixes.
  Resolved 2026-09-25, 4a3131f (session 2026-09-25-decisions-and-e2e), decided: deleted DurableQueue/replayQueue/NoOpQueue and better-sqlite3; old options kept as ignored; CLI deletes the legacy queue.db; code-archive/shared/CA-0028.
- [ ] **Agent durable queue is shared by every agent on the machine (audit part2 G2).** Default path `~/.vhyxvoid/queue.db` regardless of label/port, so a second CLI agent replays the first one's inbound requests against its own port. Default to a per-label/port file (or add a `label` column and filter). See `shared/audit-2026-09-24-part2.md` G2. **Latent since `2f90df4` (G3):** outbound responses are no longer written and nothing enqueues inbound, so the shared file currently holds nothing to cross-replay; fix together with the no-producer item below. Filed 2026-09-25 from audit part2 (session 2026-09-25-audit-part2-fixes).
  Resolved 2026-09-25, 4a3131f (session 2026-09-25-decisions-and-e2e), moot: the queue file no longer exists (queue removed); code-archive/shared/CA-0028.
