<!--
Permanent reference copy of the 2026-09-24 whole-application read-only audit
(first written to the untracked repo-root file claude-output.md; content below
is that report verbatim). Finding IDs (C1-C4, H1-H11, M1-M32, L1-L14, A1-A9,
F1-F9) are cited by context.md / decision.md / backlog.md entries.

Status of findings since the audit (update this block, not the report body):
- C1/C2 (credential logging): fixed in source, aff0c83 (api/context.md #58). Every
  call site on the report's list removed; the two remaining token logs sit in
  unreachable no-NotificationService else-branches.
- H4 (usage FK mismatch): core fixed in source, 9840c8a (with a68f355, 369f350;
  api/context.md #59). Remainder open: see api/backlog.md ("H4 remainder").
- C3/C4 (tunnel cookie rewrite, credentialed CORS reflection): short-term
  mitigation in source, ffb80a0 (docs 90d9ac3); shared/context.md #60. Full fix
  (A1, separate PSL-registered tunnel domain) deferred.
- H1 (shared rate-limit bucket, spoofable audit IPs): fixed in source, 5e255b0
  (api/decision.md, 2026-09-24, "H1"). Also found: all of /api/v1/apikeys was never
  rate-limited (limiter registered after it); fixed in the same commit.
- H5 (compressed responses truncated): fixed in source on both hub and agent, 58a98f1
  (shared/decision.md, 2026-09-24, "H5/H6"); the hub half covers already-published agents.
- H6 (nginx 1 MB body cap): fixed in source, 86397d5; needs an nginx container recreate on
  deploy (shared/backlog.md deploy note). Docs re-verified 2a7870c.
- H8 (replay window, pre-auth replay write, unsigned query, unescaped separator): fixed in
  source, 222546f (shared/decision.md, 2026-09-24, "H8/H9"). Skew 30 s, replay TTL 65 s.
- H9 (agent SSRF via absolute path): fixed in source on agent and hub, 337c60b. Correction:
  /internal/proxy is not a route to the agent (it sends a legacy "http_request" the agent
  ignores); the agent's WebSocket opener had a second, unreported variant ("@host" paths).
- H11 (guessable tunnel URLs): new slugs always carry a random suffix, 7a71695
  (api/decision.md, 2026-09-24, "H11"). Existing slugs unchanged. M18 partly overlaps (empty
  prefix now "workspace"); rest of M18 still open. Docs 867ca42.
- H2 (100 h, non-revocable access tokens): fixed in source, bd62512 (api/context.md #61, api/decision.md,
  2026-09-24, "H2"). 15 min TTL for both; guards check a Redis-cached tokenVersion/admin state
  (30 s TTL); logout/logout-all/password reset/change and admin disable/enable invalidate it;
  typed tokens enforced both ways; admin auth failures now 401 (apps/admin refreshes on 401).
  Not covered: admin logout (AdminUser has no tokenVersion), bounded by the 15 min TTL.
- H3 (key revoke/rotate/expiry didn't affect connected agents): fixed in source, 621ee3f (docs
  6b65972; hub/decision.md, 2026-09-24, "H3"). Extended AccountStatusSweep (~60 s), no pub/sub event.
- H10 (refresh race / multi-tab logout): fixed in source, 2d9530f (api, migration) + cf0c3c4
  (apps/web); api/context.md #64. Found on the way: PrismaUnitOfWork.execute() never opens a
  transaction anywhere (api/context.md #63, open).
- Everything else: open unless a later entry says otherwise.
Companion second-pass report (gaps G1-G14, stress plan, A1-A10, F1-F18): the
untracked repo-root random-audit.md.
-->
