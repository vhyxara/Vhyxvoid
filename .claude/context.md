...
41. **OPEN, newly discovered — `GracePeriodWorker` is a real, complete, correctly-written class that is never actually scheduled anywhere.** Surfaced 2026-09-13 while fixing item 6/36 (`SubscriptionPlanLimitService`'s `PAST_DUE` handling relies on this worker eventually moving a lapsed account to `SUSPENDED`). Confirmed by grep: zero references to `GracePeriodWorker` outside `apps/api/src/modules/billing/infrastructure/workers/GracePeriod.worker.ts` itself — not imported, not instantiated, `.start()` never called from `billing.plugin.ts` or anywhere else, despite the file's own header comment ("Register this in your billingPlugin after the Fastify setup") describing exactly where it should go. Practical effect: an account that goes `PAST_DUE` (Stripe payment failure — confirmed this transition itself works, `HandleStripeWebhookUseCase` sets `Account.status = PAST_DUE` and `graceEndsAt` correctly) has no automatic path to ever reaching `SUSPENDED` — it stays `PAST_DUE` indefinitely, which (per item 6's decided design) means it also keeps its real plan's limits indefinitely, not just for the intended 7-day grace window. Not fixed this session — scheduling it correctly (where in the boot sequence, whether it should run against existing stale `PAST_DUE` accounts immediately on first deploy, interaction with the also-unregistered pieces of `billing.plugin.ts`) is a real, separate, contained fix that deserves its own verification pass rather than a one-line addition tucked into a billing-model session.
