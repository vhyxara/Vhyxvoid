...
| `HUB_INTERNAL_URL` (dev only, api)                                                                                      | Intended for a `POST /internal/proxy` server-initiated agent-control feature (API → Hub → specific agent by id). No caller currently exists in `apps/api`; feature unbuilt. Low priority unless server-initiated agent communication is needed.   | Unused today                   |
...
- ~~What is `HUB_INTERNAL_URL` for?~~ **Resolved: an unbuilt server-initiated agent-control feature (`/internal/proxy`). No current caller.**
