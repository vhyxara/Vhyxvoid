...
export async function tunnelRoutes(fastify: FastifyInstance) {
  ...
  /**
   * GET /organizations/:accountId/usage/summary
   * Current period totals for the dashboard header cards.
   * Returns: total requests, active keys, active tunnels.
   * Auth: any member.
   */
  // fastify.get<{ Params: z.infer<typeof accountParamSchema> }>(
  ...
  //       totalMemberCount,
  //     ] = await Promise.all([
  //       // Total requests this month from UsageAggregate
  ...
  //     const totalRequests = Number(monthlyRequestAgg._sum?.quantity ?? 0);
