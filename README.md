...
# Flow
USER installs agent:
  npx @platform/agent --key bksr_dev_abc --port 3000 --label "api"

Agent starts:
  AgentClient.start()
  → CONNECTING
  → WS opens to wss://hub.yourplatform.com:3001/ws
  → sends { type: "agent:register", keyId: "bksr_dev_abc", label: "api", signature: "..." }
...
DEVELOPER writes frontend code:
  const tunnel = new TunnelClient({ hubUrl, keyId: "bksr_dev_abc", secret: "raw_secret", label: "api" })
